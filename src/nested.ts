...
export function getNames(questions: Question[]): string[] {
    let newArr = questions.map((question: Question): string => question.name);
    return newArr;
}
...
export function sumPoints(questions: Question[]): number {
    let total = questions.reduce((accumulator, question: Question) => {
...
export function sumPublishedPoints(questions: Question[]): number {
    let sumPublished = questions.reduce(
...
export function duplicateQuestionInArray(
    ...
    let dupeIdx = questions.findIndex((question: Question): boolean => question.id === targetId);
    let copyValue = duplicateQuestion(questions[dupeIdx].id, questions[dupeIdx])
    copyValue.id = newId;
    let newarr = [...questions]
    newarr.splice(dupeIdx+1, 0, copyValue)
    let final = (dupeIdx === -1) ? [...questions,  copyValue]: (newarr) ;
    return final
}
